...
class Parser:
    ...
    def _choice(self):
        expr = Grouping(list(self._expression()))
        ...
        while not self._is_at_end():
            ...
            right = Grouping(list(self._expression()))
    ...
    def _primary(self):
        if self._match(TokenType.DOT):
            if self._match(TokenType.STAR):
                return (
                    RandomLiteral(StarMultiplication(
                        Literal(self._token_list[self._current - 2].lexeme)))
                    if self._match(TokenType.QUESTION)
                    else StarMultiplication(Literal(self._previous().lexeme))
                )
            if self._match(TokenType.PLUS):
                return (
                    RandomLiteral(
                        PlusMultiplication(
                            Literal(
                                self._token_list[self._current - 2].lexeme)))
                    if self._match(TokenType.QUESTION)
                    else PlusMultiplication(Literal(self._previous().lexeme))
                )
            if self._match(TokenType.QUESTION):
        ...
        if self._match(TokenType.LEFT_PAREN, TokenType.LEFT_SQUARE):
            ...
            if self._match(TokenType.STAR):
                return (
                    RandomGrouping(StarMultiplicationGrouping(expr))
                    if self._match(TokenType.QUESTION)
                    else StarMultiplicationGrouping(expr)
                )
            elif self._match(TokenType.PLUS):
                return (
                    RandomGrouping(PlusMultiplicationGrouping(expr))
                    if self._match(TokenType.QUESTION)
                    else PlusMultiplicationGrouping(expr)
                )
